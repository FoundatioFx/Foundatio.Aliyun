# Agent Guidelines for Foundatio.Aliyun
...
```bash
# Build
dotnet build Foundatio.Aliyun.slnx

# Test
dotnet test Foundatio.Aliyun.slnx

# Format code
dotnet format Foundatio.Aliyun.slnx
```
...
```text
src
├── Foundatio.Aliyun                 # Aliyun provider implementations
│   ├── Storage                      # Aliyun IFileStorage implementation
tests
├── Foundatio.Aliyun.Tests           # Unit and integration tests
```
...
- Mirror the main code structure (e.g., `Storage/` tests for `src/Foundatio.Aliyun/Storage/`)
...
# Specific test file
dotnet test --filter "FullyQualifiedName~AliyunStorageTests"
